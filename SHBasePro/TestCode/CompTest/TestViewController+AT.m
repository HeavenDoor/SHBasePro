...
@implementation TestViewController (AT)

+ (BOOL)Agl_swizzleMethod:(SEL)origSel withMethod:(SEL)altSel {
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        [self Agl_swizzleMethod:@selector(test) withMethod:@selector(gl_ATTest)];
    });
}

- (void)gl_ATTest {
    NSLog(@"gl_ATTest test ...");
